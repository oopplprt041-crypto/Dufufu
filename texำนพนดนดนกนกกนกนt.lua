...
local function AddESP(player)
    if player == localPlayer or not (ESP_Normal or ESP_Team or ESP_Weapon) then return end 
...
local function RefreshESP()
    if ESP_Normal or ESP_Team or ESP_Weapon then
...
RunService.RenderStepped:Connect(function()
    if not (ESP_Normal or ESP_Team or ESP_Weapon) then return end
        ...
        if player ~= localPlayer then
            ...
            if char then
                ...
                if label and label:FindFirstChild("InfoText") and highlight and humanoid and targetHrp and myHrp then
                    ...
                    -- ==================================================
                    -- [[ ลอจิกแยกสี ESP ตามโหมดที่เลือก ]]
                    -- ==================================================
                    local targetColor = Color3.fromRGB(255, 255, 255) -- ค่าเริ่มต้นสีขาว (ESP ปกติ)
                    ...
                    if ESP_Weapon then
                        ...
                        local function checkItemName(itemName)
                            ...
                            if string.find(name, "sword") or string.find(name, "blade") or string.find(name, "katana") or string.find(name, "knife") or string.find(name, "dagger") or string.find(name, "saber") or string.find(name, "ดาบ") then
                                hasSword = true
                            elseif string.find(name, "gun") or string.find(name, "pistol") or string.find(name, "rifle") or string.find(name, "shotgun") or string.find(name, "revolver") or string.find(name, "smg") or string.find(name, "sniper") or string.find(name, "ak") or string.find(name, "m4") or string.find(name, "glock") or string.find(name, "ปืน") then
                                hasGun = true
                            end
                        end
                        
                        -- เช็คของที่ถืออยู่และในกระเป๋า
                        for _, item in pairs(char:GetChildren()) do
                            if item:IsA("Tool") then checkItemName(item.Name) end
                        end
                        ...
                        -- กำหนดสีตามอาวุธ
                        if hasSword then
                            targetColor = Color3.fromRGB(255, 50, 50) -- แดง
                        elseif hasGun then
                            targetColor = Color3.fromRGB(50, 150, 255) -- ฟ้า
                        else
                            targetColor = Color3.fromRGB(50, 255, 50) -- เขียว
                        end
                    ...
                    elseif ESP_Team then
                        ...
                        elseif player.Team ~= localPlayer.Team then
                            ...
                            targetColor = Color3.fromRGB(50, 150, 255) 
                        end
                    end
                    -- ==================================================
        ...
        if following and followTarget and followTarget.Character and followTarget.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if myHRP and myHum then
                ...
                elseif posMode == "front" then
                    myHRP.CFrame = (targetHRP.CFrame * CFrame.new(0, 0, -1.5)) * CFrame.Angles(0, math.rad(180), 0)
                    
                    if not myChar:FindFirstChild("FollowAnimPlaying") then
                        local tag = Instance.new("BoolValue", myChar)
                        tag.Name = "FollowAnimPlaying"
                        
                        local anim = Instance.new("Animation")
                        anim.AnimationId = "rbxassetid://148840371"
                        local track = myHum:LoadAnimation(anim)
                        track.Name = "FrontFollowTrack"
                        track:Play()
                    end
                end
                
                if posMode ~= "front" and myChar:FindFirstChild("FollowAnimPlaying") then
                    myChar.FollowAnimPlaying:Destroy()
                    for _, track in pairs(myHum:GetPlayingAnimationTracks()) do
                        if track.Name == "FrontFollowTrack" then track:Stop() end
                    end
                end
...
local function stopFollow()
    ...
    if myChar then
        ...
        if myChar:FindFirstChild("FollowAnimPlaying") then
            myChar.FollowAnimPlaying:Destroy()
        end
...
Tab1:Section({Title = "ระบบมองทะลุ (ESP หมวดหมู่)"})

-- ปุ่มเปิด-ปิด ESP ที่แยกออกเป็น 3 โหมดอิสระ
Tab1:Toggle({
    Title = "👁️ มองผู้เล่น (ESP ปกติ สีขาว)",
    Value = false,
    Callback = function(v)
        ESP_Normal = v
        RefreshESP()
...
Tab1:Toggle({
    Title = "🔫 มองผู้เล่น (ESP แยกตามอาวุธ)",
    Value = false,
    Callback = function(v)
        ESP_Weapon = v
        RefreshESP()
...
Window:Notify({
    Title = "BOSS69",
    Desc = "ระบบ UI Premium และ ESP ทั้ง 3 โหมดพร้อมใช้งาน!"
})
